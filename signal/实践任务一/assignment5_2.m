...
for m = times :times :(N-1)*times       %错误时间接受
    x_min = 1000000 ;   
    i1_R(m/times)  = -3  ;
    for i = -3 : 2 :3       %寻找最近x坐标，并记入i1
        if( abs( z_R(m+times) - eg*i/T) < x_min)
            i1_R(m/times)  = i ;
            x_min = abs( z_R(m+times)- eg*i/T) ;
        end
    end
    y_min = 1000000 ;
    i1_I(m/times) = -3  ;
    for i = -3 : 2 :3       %寻找最近y坐标，并记入i1
        if( abs( z_I(m+times) - eg*i/T) < y_min)
             i1_I(m/times)  = i ;
             y_min = abs( z_I(m+times) - eg*i/T) ; 
        end
    end
end
