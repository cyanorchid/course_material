%张志豪-2019270103014
%assignement4


N = 128000 ;   %序列长度
i_R = randi([-1,2],1,N);     %i实部，随机-1，0，1，2四个数
i_I = randi([-1,2],1,N);     %i虚部，随机-1，0，1，2四个数
n = (1:N);  %与序列等长向量
T = 1*10^(-6) ;     %g（t）时间
M = 16 ;            %16QAM
eg = T ;    %g(t)能量
es = 0 ;    %平均符号能量
eb = 0 ;    %平均比特能量
e_total=0;  %总能量
e_expectation=0;    %|In|^2的期望
times = 100 ;    %每1ns取样点个数
SNR = 12 ;      %信噪比
t = (1:times*N) ;       %总取样时间点向量
H = 1;          %高斯白噪声频率，单位GHZ
e_expectation = e_total/N;  %计算相关数据
es = eg * e_expectation;
eb = es / (log2(M));


 for SNR = 0:1:12
k = 2*eb/(3*10^(SNR/10));

% t_R = i_R +  wgn(1,N,k*b,'linear');   %加入噪声
% t_I = i_I +  wgn(1,N,k*b,'linear');


t_R =  awgn(i_R,SNR,'measured');     %加入噪声
t_I =  awgn(i_I,SNR,'measured');   



for i = 1:N * times

q_R(i) = t_R(ceil(i/times));    %离散化
q_I(i) = t_I(ceil(i/times));

p_R(i) = q_R(i)*g(mod(i-1,times)+1);
p_I(i) = q_I(i)*g(mod(i-1,times)+1);

r_R(i) = p_R(i);
r_I(i) = p_I(i);

end



%  r_R = p_R +  wgn(1,N*times,k*b*times,'linear');      %加入噪声
%  r_I = p_I +  wgn(1,N*times,k*b*times,'linear');  

% r_R = awgn(p_R,SNR,'measured');      %加入噪声
% r_I = awgn(p_I,SNR,'measured');  

z_R = conv(r_R,h)/times;   %匹配滤波后z（t）实部
z_I = conv(r_I,h)/times;   %匹配滤波后z（t）虚部  


i1_R = zeros(1,N);
i1_I = zeros(1,N);

for m = times :times :N*times
    x_min = 100 ;       
    i1_R(m/times)  = -3  ;
    for i = -3 : 2 :3       %寻找最近x坐标，并记入l1
        if( abs( z_R(m) - i) < x_min)
            i1_R(m/times)  = i ;
            x_min = abs( z_R(m) - i) ;
        end
    end
    y_min = 100 ;
    i1_I(m/times) = -3  ;
    for i = -3 : 2 :3       %寻找最近x坐标，并记入l1
        if( abs( z_I(m) - i) < y_min)
             i1_I(m/times)  = i ;
             y_min = abs( z_I(m) - i) ; 
        end
    end
end

num_err = 0 ;%错误数
for i = 1:N
    if(i1_R(i) ~= i_R(i)||i1_I(i) ~= i_I(i)) %计算错误数
        num_err = num_err + 1 ;
    end
end


% t0=1:N;
% plot(t0,i_R(t0),'*k');
%     hold on;
% plot(t0,r_R(t0*times),'xr');
%     hold on;

% for i=times:times:times*N
%     
% %     plot(i/times,s_R(i/times),'vk'); %缁樺埗鍙栨牱锟??
% %     hold on;
% %     plot(i/times,z_R(i),'^g'); %缁樺埗鍙栨牱锟??
% %     hold on;
% %     plot(i/times,i1_R(i/times),'<r')
% %     hold on;
% %     plot(i/times,i_R(i/times),'>b')
% end

e_expectation = e_total/N;  %计算相关数据
es = eg * e_expectation ;   
eb = es / (log2(M));    

p_16(SNR+1)=3*qfunc(sqrt(4*10^(SNR/10)/5))*(1-3/4*qfunc(sqrt(4*10^(SNR/10)/5)));
% p_16(SNR+1)=4*qfunc(sqrt(3*10^(SNR/10)/(M-1)))*(1-1/sqrt(M)) ; %理论值
p_e(SNR+1)=num_err/N ;         %实际值
% p_16(SNR+1) = 4*(1-1/sqrt(M)) * qfunc(sqrt(3*10^(SNR/10)/(M-1)));%理论误码率
end

snr_num =0:1:12;
semilogy(snr_num,p_16,'r',snr_num,p_e,'b')
xlabel('SNR');
title('16QAM错误概率理论与仿真结果 ')